...
if __name__ == "__main__":
    ...
    for r in range(9):
        ...
        labels.append(str(r))
    for r in  range(10):
        ax0.plot(output_time_vector[0:401],data_list[r][0:401],color=colors[r],
                linewidth=3, alpha=0.75,label=labels[r])
    ax0.legend(title='exponent')
    plt.axis('off')
    fig_traces.savefig('traces.png',dpi=300)
    dots = []
    for i, data in enumerate(data_list):
        dots.append(ax.plot([], [], linestyle='none', marker='h',
    ...
    anim = animation.FuncAnimation(fig_animate, animate, frames=len(
              output_time_vector), blit=False)
