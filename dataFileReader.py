...
def _get_rhotsetdicts_20160801(tsetdatadir, tsetdataprefix,
    ...
    if os.path.isdir(dirname) and os.path.isfile(outrfname) \
        ...
        print 'datadicts not found: creating new...'
...
def _update_datadicts(rdata, tsetdata, tsetdatadir, tsetdataprefix,
                chrfullname, region, res, dataset, rhomode='frac', force=False):
    """
    ...
    for key in rdata:
        ...
        if (key not in rdict) or (rdict[key] > rdata[key]):
            ...
            if (rdict[key] < rdata[key]):
                print 'Force update', key
            # Update
            rdict[key] = rdata[key]
            tdict[key] = tsetdata[key]
            nupdate += 1
...
class DataFileReader:
    ...
    def update_datadicts(self, cname, inrho, intset, force=False):
        """
