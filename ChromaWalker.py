...
def _get_edgeDataframe_triangle(nodedata, effInteraction, affinity):
    """
    Create pandas dataframe of effective interactions and affinity between
    partitions.
    Outputs only the upper triangle of interaction matrix
...
def _plot_fmat(fmat, mappingdata, res, cytobanddata=None, colorbar=False,
    ...
    #### HACK: Shift color scale to include all points
    #vmax = np.log(np.max(fmat)) / np.log(10)
    norm = colors.LogNorm(vmin=vmin, vmax=vmax)
    size = mappingdata[1] * res / 1.0e6
    fmatpad = plu._build_fullarray(fmat, mappingdata, 0.0)
    #fmatpad = (fmatpad + fmatpad.T) / 2.0
    fmatpad[fmatpad == 0.0] = np.min(fmatpad[fmatpad > 0.0])
...
class ChromaWalker:
    ...
    def _get_bestBeta(self, cname, maxdisconnected=0.1):
        ...
        if self.bestbeta[cname] is not None:
            ...
            for beta in blist:
                ...
                thissize = len(self.DFR.get_mmat(cname, beta)[0])
    ...
    def _plot_allFMCmats(self, colorbar=True):
        """
        ...
        for cname in self.cnamelist:
            ...
            fmat, mmat, cmat, mappingdata = self.DFR.get_arrays(cname,
                        beta)
            #fmat, mappingdata = self.DFR.get_fmat(cname,
                        #1.0), self.DFR.get_mappingdata(cname, 1.0)
            # Get cytobands
            ...
            f, x = _plot_fmat(fmat, mappingdata, self.res,
                    cytobanddata=cytobanddata, colorbar=colorbar, title=t)
            plotfname = os.path.join(plotdir, 'fmat-%s.pdf' % cname)
            f.savefig(plotfname)
            plt.close(f)
            t = '%s, $\\beta=%i$: $m_{ij}$' % (cname, self.bestbeta[cname])
            t = 'Chromosome %s, $\\beta=%i$' % (cname[3:], self.bestbeta[cname])
    ...
    def getAllFMCmats(self, allbeta=False, maxdisconnected=0.1):
        """
        Compute all arrays f/m/cmat for all chromosomes, at the
        highest good beta (default: less than 10% of loci excluded).
        ...
        for cname in self.cnamelist:
            ...
            if allbeta:
                for beta in self.betalist:
                    _ = self.DFR.get_cmat(cname, beta)
                bestbeta = self._get_bestBeta(cname,
                        maxdisconnected=maxdisconnected)
            else:
                bestbeta = self._get_bestBeta(cname,
                        maxdisconnected=maxdisconnected)
                print 'Chromosome %s best beta: %i' % (cname, bestbeta)
                _ = self.DFR.get_cmat(cname, bestbeta)
                print 'Chromosome %s done getting beta: %i' % (cname, bestbeta)
    ...
    def autoTsetOptimization(self, allbeta=False):
        """
    ...
    -----------------------------------------------------
        ...
        for cname in self.cnamelist:
            if allbeta:
              for beta in self.betalist:
                self.tsetOptimizerLoop(cname, beta=beta)
            else:
    ...
    def get_edgeDataframe(self, bestmeansize=1.0, goodLevels=False):
        ...
        edcsv = os.path.join(self.casedir,
                'edgedata-ms%.e-%s.csv' % (bestmeansize,
                        'good' if goodLevels else 'optimal'))
        edcsv2 = os.path.join(self.casedir,
                'edgedataTriangle-ms%.e-%s.csv' % (bestmeansize,
                        'good' if goodLevels else 'optimal'))
        if os.path.isfile(edfname):
            ...
            self.edgedata2 = _get_edgeDataframe_triangle(self.nodedata,
                        self.effInteraction, self.affinity)
            self.edgedata2.to_csv(edcsv2, index=False)
    ...
    def _plot_effectiveInteraction_inter(self, cnamelist, axis,
        ...
        axis.set_xticklabels([(c[3:] if i % 2 == 0 else ' ') for i, c in enumerate(cnamelist)], fontsize=30)
        axis.set_yticks(chrmidpts)
        axis.set_yticklabels([(c[3:] if i % 2 == 0 else ' ') for i, c in enumerate(cnamelist)], fontsize=30)
        axis.set_xlabel('genomic partitions', fontsize=20)
        axis.set_ylabel('genomic partitions', fontsize=20)
        ...
        if colorbar:
            ...
            cb.update_ticks()
            for mt in cb.ax.yaxis.get_ticklabels(which='both'):
              _ = mt.set_size(16)
            cb.ax.set_ylabel('$F_{AB}$', fontsize=24, rotation=0, labelpad=20)
        axis.set_xlim(np.min(bvals), np.max(bvals))
        axis.set_ylim(np.min(bvals), np.max(bvals))
        axis.set_aspect(1)
        axis.tick_params(axis='both', which='both', labelsize=20)
        for mt in axis.xaxis.get_ticklabels(which='both'):
          _ = mt.set_size(14)
        for mt in axis.yaxis.get_ticklabels(which='both'):
          _ = mt.set_size(14)
        #axis.xaxis.tick_top()
        plt.draw()
    ...
    def _plot_affinity_inter(self, cnamelist, axis,
        ...
        #for cb in chrbounds:
            ...
            #axis.axvline(x=cb, lw=1.0, c='k')
        axis.set_xticks(chrmidpts)
        axis.set_xticklabels([(c[3:] if i % 2 == 0 else ' ') for i, c in enumerate(cnamelist)])
        axis.set_yticks(chrmidpts)
        axis.set_yticklabels([(c[3:] if i % 2 == 0 else ' ') for i, c in enumerate(cnamelist)])
        axis.set_xlabel('genomic partitions', fontsize=20)
        axis.set_ylabel('genomic partitions', fontsize=20)
        #axis.xaxis.set_label_position('top')
        if colorbar:
            ...
            cb.set_ticklabels(np.arange(-1, 4) + 1)
            for mt in cb.ax.yaxis.get_ticklabels(which='both'):
              _ = mt.set_size(16)
            cb.ax.set_ylabel('$C_{AB}$', fontsize=24, rotation=0, labelpad=20)
        axis.set_xlim(np.min(bvals), np.max(bvals))
        axis.set_ylim(np.min(bvals), np.max(bvals))
        axis.set_aspect(1)
        for mt in axis.xaxis.get_ticklabels(which='both'):
...
if __name__ == '__main__':
    ...
    #print 'Chr17 best beta:', cw._get_bestBeta('chr17')
    ############################################
    ...
    #cw.autoTsetOptimization(allbeta=allbeta)
    #cw._plot_partitionHierarchy()
    ############################################
    ...
    cw.getGenomeEffectiveNetwork(bestmeansize=bestmeansize,
                                 goodLevels=goodLevels)
